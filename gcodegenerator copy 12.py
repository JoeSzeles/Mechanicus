...
def interpolate_parabolic(point_a, point_b, point_c, num_points):
    ...
    # Calculate the coefficients of the parabolic equation y = ax^2 + bx + c
    a = ((y_a - y_b) / ((x_a - x_b) * (x_a - x_c)) - (y_a - y_c) / ((x_a - x_b) * (x_a - x_c))) / (x_c - x_b)
    b = ((y_a - y_b) / (x_a - x_b)) - a * (x_a + x_b)
    c = y_a - a * x_a ** 2 - b * x_a

    # Interpolate points along the parabolic path
    parabolic_points = []
    for t in np.linspace(0, 1, num_points):
        x = x_a + t * (x_b - x_a)
        y = a * x ** 2 + b * x + c
        parabolic_points.append((x, y))

    return parabolic_points

# Define the control points A, C, and B
point_a = (0, 0)
point_c = (3, 1)  # Adjust these points as needed
point_b = (6, 0)

# Interpolate the parabolic path
num_points = 50
parabolic_points = interpolate_parabolic(point_a, point_b, point_c, num_points)

# Extract x and y coordinates for plotting
x_coords = [point[0] for point in parabolic_points]
y_coords = [point[1] for point in parabolic_points]

# Plot the parabolic path
plt.plot(x_coords, y_coords, marker='o')
plt.title('Parabolic Path Interpolation')
...
def shapes_2_gcode(shapes, z_draw_value, material_height_value):
    ...
    for i, shape in enumerate(shapes):
        ...
        if i < len(shapes) - 1:
            ...
            if end == next_start:
                # End of the current shape is connected to the start of the next shape

                # Split the line into multiple segments and interpolate parabolic paths for each segment
                num_segments = len(shape) - 1
                for j in range(num_segments):
                    segment_start = shape[j]
                    segment_end = shape[j + 1]

                    # Integrate parabolic path interpolation for the segment
                    parabolic_points = interpolate_parabolic(segment_start, segment_end, (x_c, y_c), num_points=50)

                    # Generate G-code for the parabolic path segment
                    for point in parabolic_points:
                        commands.append(g_string(point[0], point[1], z_draw_value, f'G1', draw_speed, feed_rate=draw_speed))

                    # Ensure a continuous path by connecting the last interpolated point to the start of the next segment
                    commands.append(g_string(segment_end[0], segment_end[1], z_draw_value, f'G1', draw_speed, feed_rate=draw_speed))

            else:
                # End of the current shape is not connected to the start of the next shape
...
def generate_gcode(svg_path, gcode_path, z_draw_value, material_height_value):
    ...
    plot_toolpath(commands, z_draw_value, material_height_value)
# Rest of your code...
