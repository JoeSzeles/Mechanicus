...
def generate_mosaic():
    ...
    # Generate the points for the hyperbolic spiral
    a = 0.1  # Adjust the scale parameter as needed
    n_points = 1000  # Number of points to generate

    t_values = np.linspace(0.01, 2 * np.pi, n_points)  # Adjust the range of t as needed
    x_values = a * np.cosh(t_values)
    y_values = a * np.sinh(t_values)
