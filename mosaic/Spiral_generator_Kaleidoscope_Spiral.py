...
def generate_spiral():
    spiral_n_spiral = int(spiral_n_points_entry.get())
    # Get input values for 'a' and 'b'
    spiral_a = float(spiral_a_entry.get())
    spiral_b = float(spiral_b_entry.get())
    t_values = np.linspace(0, 4 * np.pi, spiral_n_spiral)  # Adjust the range of t as needed
    r_values = spiral_a * np.exp(spiral_b * t_values)
...
def save_mandala():
    ...
    # Generate Mandala
    x_values, y_values = generate_spiral()
...
window = tk.Tk()
window.title("Kaleidoscope Spirals")
...
# Create "Save Mandala" button
save_mandala_button = ttk.Button(window, text="Save Mandala", command=lambda: save_mandala())
save_mandala_button.grid(row=11, column=2, columnspan=2)

result_label = ttk.Label(window, text="")
result_label.grid(row=1, column=6, columnspan=2)

# Add input fields for 'a' and 'n_points'
spiral_a_label = ttk.Label(window, text="spiral Value of a")
spiral_a_label.grid(row=5, column=3)
spiral_a_entry = ttk.Entry(window)
spiral_a_entry.grid(row=5, column=4)
spiral_a_entry.insert(0, "3")  # spiral_Default value for 'a'

spiral_b_label = ttk.Label(window, text="spiral_ Value of 'b':")
spiral_b_label.grid(row=6, column=3)
spiral_b_entry = ttk.Entry(window)
spiral_b_entry.grid(row=6, column=4)
spiral_b_entry.insert(0, "0.2")  # spiral_Default value for 'b'

spiral_n_points_label = ttk.Label(window, text="spiral_Number of Points:")
spiral_n_points_label.grid(row=7, column=3)
spiral_n_points_entry = ttk.Entry(window)
spiral_n_points_entry.grid(row=7, column=4)
spiral_n_points_entry.insert(0, "100")  # spiral_Default value for 'n_points'
