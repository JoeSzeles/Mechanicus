...
# Function to generate the Doppler spiral
def generate_mosaic():
    ...
    t_values = np.linspace(2, 6 * np.pi, n_points)
    
    # Calculate x and y coordinates for Fraser's spiral
    x_values = a * t_values * np.cos(t_values)
    y_values = a * t_values * np.sin(t_values)
    
    # Create a new figure
    fig, ax = plt.subplots(figsize=(6, 6), dpi=80)
    
    # Plot Fraser's spiral
    ax.plot(x_values, y_values, label="Fraser's Spiral")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_title("Fraser's Spiral")
    ax.legend()
