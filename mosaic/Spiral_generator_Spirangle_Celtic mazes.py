...
def celtic_mazes_points(N, angle_increment, length_increment):
    x_values = []
    y_values = []
    x, y = 0, 0
    for _ in range(N):
        ...
        x, y = x_end, y_end
        angle_increment += 90  # Rotate by 90 degrees for Celtic maze pattern
...
# Function to generate the Catherine Wheel pattern mosaic
def generate_mosaic():
    ...
    # Generate points for the Catherine Wheel pattern
    x_values, y_values = celtic_mazes_points(N, angle_increment, length_increment)

    # Create a new figure with the desired size in inches
    fig, ax = plt.subplots(figsize=(16, 16), dpi=80)

    # Create a loop to draw concentric rectangles with Catherine Wheel pattern
    for step in range(Nsteps):
        ...
        # Plot the mosaic without connecting lines (Catherine Wheel pattern outline)
        current_spiral = []
    ...
    svg_filename = "concentric_rectangles_catherine_wheel.svg"
    plt.savefig(svg_filename, format="svg", bbox_inches="tight", dpi=300)
    plt.close()

    # Convert the SVG file to PDF (if needed)
    pdf_filename = "concentric_rectangles_catherine_wheel.pdf"
