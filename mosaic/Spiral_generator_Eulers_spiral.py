...
def generate_mosaic():
    ...
    # Generate the points for Euler's spiral
    a = 1  # Adjust the scale parameter as needed
    n_points = 1000  # Number of points to generate
    
    t_values = np.linspace(1, 7 * np.pi, n_points)  # Adjust the range of t as needed
    x_values = a * np.cumsum(np.cos(t_values) * np.sqrt(t_values))
    y_values = a * np.cumsum(np.sin(t_values) * np.sqrt(t_values))
