...
def generate_mosaic():
    ...
    a = 3 # Adjust the scale parameter as needed
    n_points = 1000  # Number of points to generate
    
    t_values = np.linspace(0, 5 * np.pi, n_points)  # Adjust the range of t as needed
    x_values = a * np.sqrt(t_values) * np.cos(t_values)
    y_values = a * np.sqrt(t_values) * np.sin(t_values)
