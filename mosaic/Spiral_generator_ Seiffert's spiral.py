...
def seifferts_spiral_points(a, b, n_points):
    t_values = np.linspace(0, 10 * np.pi, n_points)  # Adjust the range of t as needed
    r_values = a * np.exp(b * np.sin(t_values))
...
def generate_mosaic():
    ...
    x_values, y_values = seifferts_spiral_points(a, b, n_points)
...
window = tk.Tk()
window.title("Seiffert's Spirals")
