...
def generate_mosaic():
    ...
    # Generate the points for the Fibonacci spiral
    a = 2  # Adjust the scale parameter as needed
    n_points = 1000  # Number of points to generate

    phi = (1 + np.sqrt(5)) / 2  # Golden ratio

    t_values = np.linspace(0, 5 * np.pi, n_points)  # Adjust the range of t as needed
    r_values = a * np.sqrt(t_values)  # Adjust the scaling factor as needed
...
window = tk.Tk()
window.title("FIBONACCI Spirals")
