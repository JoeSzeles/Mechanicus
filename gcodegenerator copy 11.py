...
def shapes_2_gcode(shapes, z_draw_value, material_height_value):
    ...
    for i, shape in enumerate(shapes):
        ...
        if i < len(shapes) - 1:
            ...
            if end == next_start:
                ...
                # Integrate parabolic path interpolation
                point_a = (start[0], start[1])
                point_c = (x_c, y_c)
                point_b = (end[0], end[1])

                # Calculate the coefficients of the parabolic equation y = ax^2 + bx + c
                a = ((point_a[1] - point_b[1]) / ((point_a[0] - point_b[0]) * (point_a[0] - point_c[0])) - (point_a[1] - point_c[1]) / ((point_a[0] - point_b[0]) * (point_a[0] - point_c[0]))) / (point_c[0] - point_b[0])
                b = ((point_a[1] - point_b[1]) / (point_a[0] - point_b[0])) - a * (point_a[0] + point_b[0])
                c = point_a[1] - a * point_a[0] ** 2 - b * point_a[0]
    ...
    # Add the parabolic points to the commands list
    for x, y in parabolic_points:
        commands.append(g_string(x, y, z_draw_value, f'G1', draw_speed, feed_rate=draw_speed))
