...
def shapes_2_gcode(shapes, z_draw_value, material_height_value):
    ...
    for i, shape in enumerate(shapes):
        ...
        parabolic_points = calculate_parabolic_path(start, end, z_draw_value, material_height_value)
        
        # Iterate through the parabolic points and generate G-code
        for point in parabolic_points:
            x, y, z = point
            commands.append(g_string(x, y, z, f'G1', draw_speed, feed_rate=draw_speed))
            
        # Lift at the end of the shape
        commands.append(g_string(end[0], end[1], zTravel, "G0", feed_rate=travel_speed))
...
def calculate_parabolic_path(start, end, z_draw_value, material_height_value):
    # Calculate the coordinates of point C (the middle point)
    x_c = (start[0] + end[0]) / 2
    y_c = (start[1] + end[1]) / 2
    ...
    # Calculate the parabolic path points
    parabolic_points = []
    for t in range(0, 101):  # You can adjust the number of points for the parabola
        t_normalized = t / 100.0
        x = (1 - t_normalized) * ((1 - t_normalized) * start[0] + t_normalized * x_c) + t_normalized * ((1 - t_normalized) * x_c + t_normalized * end[0])
        y = (1 - t_normalized) * ((1 - t_normalized) * start[1] + t_normalized * y_c) + t_normalized * ((1 - t_normalized) * y_c + t_normalized * end[1])
        z = (1 - t_normalized) * ((1 - t_normalized) * material_height_value + t_normalized * z_draw_value) + t_normalized * ((1 - t_normalized) * z_draw_value + t_normalized * material_height_value)
        parabolic_points.append((x, y, z))
