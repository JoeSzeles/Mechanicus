...
def shapes_2_gcode(shapes, Z_line_start, Z_line_center, Z_line_end, Z_line_speed, draw_speed, gradient_length, zTravel):
    commands = []
    
    for shape in shapes:
        current_z = Z_line_start  # Reset to Z_line_start at the beginning of each line segment
        
    for i, shape in enumerate(shapes):
        start = shape[0]
        end = shape[-1]

        # Calculate Z-axis values along the line segment
        z_values = calculate_z_values(start, end, num_points, Z_line_start, Z_line_end, gradient_length)

        if i < len(shapes) - 1:
            next_start = shapes[i + 1][0]
            if end == next_start:
                # end of current shape is connected to start of next shape
                for j, z in zip(shape, z_values):
                    commands.append(g_string(j[0], j[1], z, 'G1', draw_speed, feed_rate=draw_speed))
            else:
                # end of current shape is not connected to start of next shape
                for j, z in zip(shape, z_values):
                    commands.append(g_string(j[0], j[1], z, 'G1', draw_speed, feed_rate=draw_speed))
                commands.append(g_string(end[0], end[1], zTravel, 'G1', draw_speed, feed_rate=draw_speed))
        else:
            # last shape
            for j, z in zip(shape, z_values):
                commands.append(g_string(j[0], j[1], z, 'G1', draw_speed, feed_rate=draw_speed))
...
def generate_gcode():
    ...
    print("Shapes:", shapes)
    # Pass the Z-line parameters and draw_speed when calling shapes_2_gcode
    commands = shapes_2_gcode(shapes, z_line_start, z_line_center, z_line_end, z_line_speed, draw_speed, gradient_length, zTravel)
