...
ser = serial.Serial('COM3', 115200)  # Change 'COM4' to the appropriate COM port and baud rate
# Open the webcam
...
while running:
    ...
    if green_x is not None and green_y is not None:
        ...
        # Calculate the distance between the CNC rectangle and the center of the green dot
        dist = distance((cnc_rect.centerx, cnc_rect.centery), (x_abs, y_abs))

        if dist > 0:
            # Calculate the unit vector towards the center of the green dot
            dx = (x_abs - cnc_rect.centerx) / dist
            dy = (y_abs - cnc_rect.centery) / dist

            # Move the CNC rectangle directly towards the center of the green dot
            ...
            # Update the G-code position based on the new CNC rectangle position
            XG = cnc_rect.centerx
            YG = cnc_rect.centery
