...
cnc_width, cnc_height = 150, 150  # in millimeters
...
def move_to_position(x, y, speed):
    # Limit movement within the 130 x 130 mm area
    x = max(0, min(x, 150))
    y = max(0, min(y, 150))
...
def tracer():
    ...
    while True:
        ...
        if green_x is not None and green_y is not None:
            ...
            dx = int(center_x - green_x)/3
            dy = int(center_y - green_y)/3
            print (dx,dy)
            # Move the CNC by the calculated movement
            move_to_position(dx, dy, speed=1000)
