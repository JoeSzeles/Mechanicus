    ...
    if green_x is not None and green_y is not None:
        ...
        if dist > 0:
            ...
            cnc_rect.top = max(0, min(cnc_rect.top, WINDOW_SIZE[1] - cnc_rect.height))
                # Send data as a string in the format: "x,y,speed"
            serial_data = f"{cnc_rect.centerx},{cnc_rect.centery},{cnc_speed}\n"
            ser.write(serial_data.encode())  # Send the data over serial
